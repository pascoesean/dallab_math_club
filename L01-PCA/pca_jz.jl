...
# ╔═╡ f1b37a52-4c4f-11f0-2266-7b8133272141
begin
	using CSV
	using DataFrames
	using CairoMakie
...
begin
	...
	penguins = CSV.read(download(url), DataFrame)
	
	# convert strings to numeric types
	floats = [:bill_length_mm, :bill_depth_mm]
	ints = [:flipper_length_mm, :body_mass_g]
	
	penguins[!, floats] = tryparse.(Float64, penguins[!, floats])
	penguins[!, ints] = tryparse.(Int16, penguins[!, ints])

	# convert all "NA" and nothing => missing
	for col_name in names(penguins)
		penguins[!, col_name] = replace(penguins[!, col_name], "NA" => missing, nothing => missing)
	end
...
# ╔═╡ a459c45d-6251-46f6-a250-c2f273550aa7
begin
	# extract numeric columns only
	num_mask = map(t -> t <: Union{Missing, Number}, eltype.(eachcol(penguins)))
	X = select(penguins, names(penguins)[num_mask])

	# remove year because that is barely a numerical feature
	X = X[!, 1:end-1]
...
# ╔═╡ 689cf4e0-1145-4934-ba09-e24d3cce3f16
md"""
## 👩‍💻 through the MultivariateStats.jl package 
"""

# ╔═╡ d0eca316-4590-40c9-98e0-05a2f5df9b21
begin
	# transpose (for some reason. i'm looking at you MultivariateStats.jl 🙈)
	X_T = X'
	pca = fit(PCA, X_T; maxoutdim=n_comps)
...
manifest_format = "2.0"
project_hash = "141c3641f3d6d6ea26b7991f9ad880a9edd3429f"
